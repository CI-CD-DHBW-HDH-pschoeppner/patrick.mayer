...
describe("validate todo", () => {
    ...
    const result = validateTodo(
      {
        id: "2h5bs",
        value:
          "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat, sed diam voluptua. At vero eos et accusam et justo duo dolores et ea rebum. Stet clita kasd gubergren, no sea takimatae",
        done: false,
      },
      todoList
    );
  ...
  it("value < 255 returns true", () => {
    const result = validateTodo(
      { id: "2h5bs", value: "Test", done: false },
      todoList
    );
  ...
  it("todo (casesensitive) already in todolist returns false", () => {
    const result = validateTodo(
      { id: "2h5bs", value: "Hund Gassi gehen", done: false },
      todoList
    );
    expect(result).toBe(false);
  });
  ...
  it("todo not in todolist returns true", () => {
    const result = validateTodo(
      { id: "2h5bs", value: "hund des Nachbarn gassi gehen", done: false },
      todoList
    );
    expect(result).toBe(true);
  });
